...
function App() {
  ...
  const setStageTwoTasks = (newTasks) => {
    // array
    ...
    setStages([...stages.filter((s) => s.name !== activeStage.name), newStage]);
  };
      ...
      <div className="App ">
        ...
        <div className="flex flex-row gap-x-5 justify-center mt-12">
          <div className="max-w-lg">
            <Header />
            <Stage
              thisStageisOpen={true}
              tasks={tasks}
              setTasks={setTasks}
              type={1}
              showAdd={showAddBucket}
              setShowAdd={setShowAddBucket}
              formIsOpen={formIsOpen}
              toggleShowAdd={toggleShowAddBucket}
            />
          </div>

          {tasks.filter((task) => task.active)[0] && (
            <div className="max-w-lg w-lg">
              {stages.filter((stage) => stage.active)[0] ? (
                <Stage
                  formIsOpen={formIsOpen}
                  stageName={stages.filter((stage) => stage.active)[0].name}
                  type={2}
                  tasks={stages.filter((stage) => stage.active)[0].tasks}
                  setTasks={setStageTwoTasks}
                  showAdd={showAddTask}
                  setShowAdd={setShowAddTask}
                  toggleShowAdd={toggleShowAddTask}
                />
              ) : (
